...
public:
	DegreeDistribution(Graph &g)
	{
		...
		for(EdgeId i=0;i<m;i++)
		{
			...
			if(buffer.first >= n) std::cerr<<" Buffer.First >= n! i = "<<i <<"  buffer.first = "<<buffer.first <<"  n = "<<n<<std::endl;
			if(buffer.second >= n) std::cerr<<" Buffer.Second >= n! i = "<<i <<"  buffer.second = "<<buffer.second <<"  n = "<<n<<std::endl;
			nodes[buffer.first]++;
			nodes[buffer.second]++;
