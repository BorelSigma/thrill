...
namespace ListRank2 
{
	...
	auto fuse(thrill::Context& ctx, std::vector<NodeValuePair> &vecV, std::vector<NodeValuePair> &IS)
	{
		...
		auto concatTmp = Distribute(ctx, IS, 0);
	...
	static void runParallel(thrill::Context& ctx, std::vector<Edge> edges, std::string output) 
	{
		...
		while(sizeV > 1)
		{
			...
			auto tmp0 = Distribute(ctx,nodes,0);			
			...
			independentSets.push_back(IS.Gather(0));	
		...
		if(ctx.my_rank() == 0) 
		{
			...
			fsoutput.open(output,std::ios::out);
